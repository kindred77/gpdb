 ...
 *
 * In addition to expanding a partitioned table to include its partitions, we
 * also use this function to expand appendoptimized tables to their auxiliary
 * tables if the AO_AUX_ONLY option is passed. This is something of a misnomer
 * because we do not return the originally input AO table, so it's a
 * replacement instead of strictly an expansion.
 */
static List *
			...
			foreach (lc, vacrels)
			{
				...
				else
				{
					ereport(classForm->relispartition ? LOG : WARNING,
							(errmsg("skipping \"%s\" --- it is not an append-optimized table",
									NameStr(classForm->relname))),
